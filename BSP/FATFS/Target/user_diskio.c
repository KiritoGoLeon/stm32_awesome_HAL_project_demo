...
#define FLASH_SECTOR_COUNT    ((12*1024*1024)/FLASH_SECTOR_SIZE)//12????MB
...
)
{
  /* USER CODE BEGIN INIT */
    ...
    if(SPI_FLASH_ReadID() == sFLASH_ID){
     Stat &= ~STA_NOINIT;
      printf("fatfs flash init begin ok---\r\n");
    }else{
...
)
{
  /* USER CODE BEGIN READ */
		...
		//printf("fatfs readsector:%x  count:%x\r\n",sector <<12, count<<12);
    return RES_OK;
  /* USER CODE END READ */
}
...
)
{
  ...
  /* USER CODE HERE */
	 uint32_t write_addr; 
	 //printf("------- this is fatfs flash begin write ---\r\n");
...
)
{
  /* USER CODE BEGIN IOCTL */
    DRESULT res = RES_ERROR;
		...
		res = RES_OK;
    //printf("------- this is fatfs flash get state :%d--- %d\r\n",cmd,res);
